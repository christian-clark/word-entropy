...
def print_samples(surps, ids, word_final, tokenizer):
...
def main(args):
    ...
    header = "word"
    addl_cols = ','.join("sample"+str(i) for i in range(1, num_samples+1))
    header = ','.join([header, addl_cols])
    print(header)
    for story in stories:
        ...
        # story_surps will ultimately be a word x sample matrix with samples
        # for all the words in the story
        story_surps = torch.empty(size=(0, num_samples))
    ...
    #                debug("topk next tokens (after ix conversion):", topk)
            ...
            window_surps = -window_log_probs
            story_surps = torch.cat([story_surps, window_surps], dim=0)
        # end loop over windows in a story
        print_samples(story_surps, story_ids, story_word_final, tokenizer)
