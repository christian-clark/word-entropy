...
class Window:
    def __init__(self, input_ids, attn_mask, word_initial, start_ix):
    ...
    def __str__(self):
        s = "Window:\n\tInput_ids: {}\n\tAttn_mask: {}\n\Word_initial: {}\n\tStart_ix: {}".format(
            self.input_ids, self.attn_mask, self.word_initial, self.start_ix
        )
        return s
...
def main():
    ...
    for story in stories:
        ...
        for ix in range(len(half_window_start_ixs)-2):
            ...
            window = Window(
                input_ids=ids[w_start:w_end],
                attn_mask=attn[w_start:w_end],
                word_initial=word_initial[w_start:w_end],
                start_ix=start_ix
            )
