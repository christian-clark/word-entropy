...
def get_space_subword_idx(tokenizer, vocab_size):
    ...
    return torch.tensor(space_idx).cuda(), torch.tensor(subword_idx).cuda()
...
def main(args):
        ...
        for window in windows:
            ...
            for batch in range(num_batches):
                ...
                if start_ix > 0:
                    ...
                    prefix_output = model(
                        input_ids=prefix_id.cuda(),
                        attention_mask=prefix_attn.cuda(),
                        use_cache=True
                    )
                ...
                for i in range(start_ix, len(ids)):
                    ...
                    curr_log_prob = torch.zeros(samples_per_batch).cuda()
    ...
    #                debug("topk next tokens (after ix conversion):", topk)
                    ...
                    ix_filtered = ix * is_subword
                    eow = torch.full((samples_per_batch, 1), EOW_IX).cuda()
